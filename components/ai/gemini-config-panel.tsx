        ...
        <div className='space-y-0.5'>
          ...
          {activeTab === 'keys' && (
            <p className='text-[11px] text-white/40'>
              Default policy: 30 days expiration. Keys are stored locally in your browser and never transmitted to Resumate servers.
            </p>
          )}
        </div>
      </div>
          ...
          <form onSubmit={handleAddKey} className='p-4 rounded-xl bg-[#121215] border border-white/8 space-y-3'>
            ...
            <div className='flex items-center justify-between pt-1'>
              <span className='text-[11px] text-white/40'>
                Auto-stored for 30 days in browser storage
              </span>
              <button
                type='submit'
              ...
              return (
                <div
                  ...
                  <div className='flex items-center gap-3 min-w-0'>
                    ...
                    <div className='space-y-0.5 min-w-0'>
                      <div className='flex items-center gap-2'>
                        ...
                        {isActive && (
                          <span className='px-1.5 py-0.2 rounded text-[9px] font-bold bg-accent/10 text-accent border border-accent/20'>
                            Active
                          </span>
                        )}
                      </div>
                      <div className='flex items-center gap-3 text-[11px] text-white/40 font-mono'>
                        <span>{k.key.slice(0, 4)}••••{k.key.slice(-4)}</span>
                        <span>•</span>
                  ...
                  <div className='flex items-center gap-3 self-end sm:self-center shrink-0'>
                    <span className='text-[11px] font-medium text-cyan-400/90 bg-cyan-500/10 border border-cyan-500/20 px-2.5 py-1 rounded-lg'>
                      {daysLeft}
                    </span>
        ...
        <div className='space-y-4'>
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-xl border text-left flex items-center justify-between transition-all ${
                    isSelected
                      ? 'bg-accent/10 border-accent/40 text-white shadow-md'
                      : 'bg-[#121215] border-white/8 text-white/60 hover:text-white hover:border-white/15'
                  }`}>
          ...
          <div className='p-4 rounded-xl bg-[#121215] border border-white/8 flex items-center justify-between'>
            <div className='space-y-0.5'>
              ...
              <span className='text-[11px] text-white/40'>
                Automatically switch to another non-exhausted Gemini model if quota limit is reached.
              </span>
            </div>
            <button
              type='button'
              ...
              className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full transition-colors duration-200 ${
                autoSwitch ? 'bg-accent' : 'bg-white/15'
              }`}>
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition duration-200 mt-0.5 ml-0.5 ${
                  autoSwitch ? 'translate-x-4' : 'translate-x-0'
                }`}
              />
            </button>
          </div>
        ...
        <div className='space-y-4'>
          ...
          <div className='p-4 rounded-xl bg-[#121215] border border-white/8 flex items-center justify-between'>
            <div className='space-y-0.5'>
              ...
              <span className='text-[11px] text-white/40'>
                Re-display the first-time onboarding guide and feature walk-through.
              </span>
            </div>
            <button
              type='button'
